...
# 5. Report.
#
# Historical note: an earlier build also wrote a separate
# "Start Chrome (debug).ps1" launcher to dist/. We removed it
# because the runner itself calls ensure_chrome_running() at
# startup (see runner_app.py:do_run) and launches Chrome with
# the right flags whenever it's not already reachable on the
# debug port. Shipping a second clickable file confused end users.
