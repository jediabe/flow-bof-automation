...
# Historical note: an earlier build also bundled a separate
# "Start Chrome (debug).command" launcher. We removed it because
# the runner itself calls ensure_chrome_running() (see
# runner_app.py:do_run) which launches Chrome with the correct
# flags whenever it's not already reachable on the debug port.
# Shipping a second clickable file just confused end users into
# running them in the wrong order.
